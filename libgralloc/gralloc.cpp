...
static int init_gpu_area_locked(private_module_t* m)
{
    ...
    gpu = open("/dev/msm_hw3dm", O_RDWR, 0);
...
#if 0
				if (stat("/dev/pmem_camera", &stbuf) == 0)
					fd = open("/dev/pmem_camera", O_RDWR, 0);
				else
#endif
        ...
        buffer_handle_t* pHandle, int* pStride)
{
            ...
            format == HAL_PIXEL_FORMAT_YCbCr_422_SP) 
    {
        ...
        int vstride;
		LOGD("Protecting postview from failure, gralloc_alloc fails.");
		return -EINVAL;
